...
def generator_from_index(X, Y, index_path, k, batch_size, search_k=-1, precompute=True, verbose=1):
    if k >= X.shape[0] - 1:
    ...
    if Y is None:
        if precompute:
            ...
            neighbour_matrix = extract_knn(X, index_path, k=k, search_k=search_k, verbose=verbose)
            return KnnTripletGenerator(X, neighbour_matrix, batch_size=batch_size)
        else:
            index = AnnoyIndex(X.shape[1])
            index.load(index_path)
            return AnnoyTripletGenerator(X, index, k=k, batch_size=batch_size, search_k=search_k)
    else:
        if precompute:
            if verbose > 0:
                print('Extracting KNN from index')
            ...
            index = AnnoyIndex(X.shape[1])
            index.load(index_path)
            return LabeledAnnoyTripletGenerator(X, Y, index, k=k, batch_size=batch_size, search_k=search_k)
